...
import pushSubscriptionRepositories from "../repositories/pushSubscription.repositories.js";
...
async function store(data) {
  ...
  if(post.userId !== data.userId) {
    await db.sequelize.transaction(async t => {
      ...
      // 게시글 작성자의 푸시 정보 획득
      const pushSubscriptions = await pushSubscriptionRepositories.findByUserId(t, post.userId);
      ...
      const pushList = pushSubscriptions.map(async pushSubscription => {
        ...
        try {
          ...
          if(error.statusCode === 410) {
            await pushSubscriptionRepositories.hardDestory(t, pushSubscription.id);
          }
        }
      });
      // 병결처리 완료 확인
